---
title: Licence
tags: ["footernav_en"]
updated: 2026-02-18T12:34:00+0800
eleventyComputed:
  desc: "{{ sitemeta[page.lang].siteName }}'s licence notice."
---
This website, {{ sitemeta.siteDomain }}, its source code and its content are licensed under a dual licence model. This means different licences apply to different parts of this website.

## Code Licence

Unless specified otherwise, the source code that powers this website—including the HTML, CSS, JavaScript, templates, and build scripts—is licensed under the [**GNU Affero General Public License v3.0 (AGPL-3.0)**](https://www.gnu.org/licenses/agpl-3.0.en.html). This applies to the technical implementation and design of the site.

You are free to:

- Use the code for any purpose, including commercial projects.
- Modify and distribute the code.
- Create derivative works based on the code.
- Use the code privately or commercially.
- Run the software on your own servers.

Under the following conditions:

- Include a copy of the original licence text and copyright notice.
- If you modify and distribute the software, you must make the source code available under the same AGPL licence.
- If you modify and distribute the software, you must document the changes made to the software.
- If you run the software as a service, users must be able to access the source code.
- The authors are not liable for any damages.

The complete source code and licence text of this website can be found on:
- [My self-hosted Gitea instance](https://git.helenchong.dev/helenchong/helenchong-blog)
- [SourceTube](https://source.tube/helenchong/helenchong-blog)
- [Codeberg](https://codeberg.org/helenchong/helenchong-blog)
- [GitHub](https://github.com/helenclx/helenchong-blog)

## Content Licence

Unless specified otherwise, all written and graphical content on this website — including articles, blog posts, images, videos and creative works — is licensed under [**Creative Commons Attribution-ShareAlike 4.0 International (CC BY-SA 4.0)**](https://creativecommons.org/licenses/by-sa/4.0/).

You are free to:

1. **Share** — copy and redistribute the material in any medium or format for any purpose, even commercially.
2. **Adapt** — remix, transform, and build upon the material for any purpose, even commercially.
3. The licensor cannot revoke these freedoms as long as you follow the license terms.

Under the following terms:

1. **Attribution** — You must give [appropriate credit](https://creativecommons.org/licenses/by-sa/4.0/#ref-appropriate-credit), provide a link to the license, and [indicate if changes were made](https://creativecommons.org/licenses/by-sa/4.0/#ref-indicate-changes). You may do so in any reasonable manner, but not in any way that suggests the licensor endorses you or your use.
2. **ShareAlike** — If you remix, transform, or build upon the material, you must distribute your contributions under the [same license](https://creativecommons.org/licenses/by-sa/4.0/#ref-same-license) as the original.
3. **No additional restrictions** — You may not apply legal terms or [technological measures](https://creativecommons.org/licenses/by-sa/4.0/#ref-technological-measures) that legally restrict others from doing anything the license permits.

